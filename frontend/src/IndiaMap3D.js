...
function IndiaMap3D({ theme }) {
  const [selectedState, setSelectedState] = useState(null);
  const [bubblePosition, setBubblePosition] = useState({ x: 0, y: 0 });
  const mapRef = useRef(null);

  // Move these objects inside useMemo to prevent recreation on every render
  ...
  const stateInfoDatabase = useMemo(() => ({
    // States
    'Andhra Pradesh': {
      name: 'Andhra Pradesh',
  ...
  }, [theme, stateInfoDatabase, stateIdToNameMap]);

  useEffect(() => {
    // Initialize 3D map
    if (mapRef.current) {
      const cleanup = initIndiaMap();
      const currentMapRef = mapRef.current; // Store ref value to avoid warning
      ...
      return () => {
        ...
        // Use the stored ref value instead of accessing mapRef.current directly
